...
export const SignIn: React.FC<{ onSignUp: () => void; onForgot: () => void; }> = ({ onSignUp, onForgot }) => {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold text-center text-amber-700 mb-6">Sign In</h2>
        ...
        <form className="space-y-4">
          ...
          <button type="submit" className="w-full bg-amber-600 text-white py-2 rounded hover:bg-amber-700 transition-colors font-semibold">Sign In</button>
        </form>
        <div className="flex justify-between mt-4 text-sm">
          <button className="text-amber-600 hover:underline" onClick={onForgot}>Forgot password?</button>
          <button className="text-amber-600 hover:underline" onClick={onSignUp}>Create account</button>
        </div>
      </div>
    </div>
  );
